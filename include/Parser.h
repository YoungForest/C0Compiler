...
class Parser
{
    public:
        ...
        void varietyDenote(SymbolTable *table, int type, string ident); // <变量说明>  ::= <变量定义>;{<变量定义>;}

        // struct symbolItem* defineReturnFunction(); // <有返回值函数定义>  ::=  <声明头部>‘(’<参数>‘)’ ‘{’<复合语句>‘}’
        void defineReturnFunction(int type, string ident);
        ...
        void assignStatement(string ident); // <赋值语句>   ::=  <标识符>＝<表达式>|<标识符>‘[’<表达式>‘]’=<表达式>
